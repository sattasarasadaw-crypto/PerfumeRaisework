...
- ~~`firebase.json` ของ hosting ไม่กัน `config.local.js` ออกจาก deploy โดยตั้งใจ — เพราะเป็นคีย์ของหลักสูตรที่ยอมรับความเสี่ยงได้ และเกณฑ์การบ้านข้อ "ปุ่ม AI ใช้งานจริงบนเว็บออนไลน์" ต้องการให้ทำงานได้จริงบนเว็บสาธารณะ (ไม่ใช่แค่ในเครื่อง) — คีย์ยังไม่ขึ้น GitHub เหมือนเดิมเสมอ~~
  ...
  > 🔴 **แก้ไขแล้วในสัปดาห์ที่ 9 — การตัดสินใจข้อนี้ผิด**
  > การยอมให้ `config.local.js` ขึ้น hosting ทำให้ใครก็ได้เปิด
  > `https://sattasarasada-perfume.web.app/config.local.js` แล้วก๊อปคีย์ของหลักสูตรไปใช้ได้ทันที
  > (ตรวจเจอตอนทำ[การบ้านที่ 4](../Submission-RAISE-M2-HW4/README.md) — ตอนนั้นตอบ HTTP 200 พร้อมคีย์เต็ม ๆ)
  >
  > `.gitignore` กันได้แค่ GitHub **ไม่ได้กันการ deploy** — ต้องกันที่ `firebase.json` อีกชั้น
  > ตอนนี้กันแล้ว ไฟล์ตอบ 404 และการหาคีย์ย้ายไปที่ `public/ai-key.js`
  > (sessionStorage → ไฟล์ในเครื่อง → ถามผู้ใช้) พร้อมเทสต์อัตโนมัติ T6 กันพลาดซ้ำ
  >
  > **คีย์เดิมที่เคยเปิดสาธารณะต้องถือว่าใช้ไม่ได้แล้ว** — ดู [`../BACKLOG.md`](../BACKLOG.md) ข้อ 1.2
