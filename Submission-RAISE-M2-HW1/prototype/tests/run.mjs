...
const prototypeDir = path.join(here, "..");
const repoRoot = path.join(prototypeDir, "..", "..");
const reportJson = path.join(prototypeDir, "test-results", "report.json");
...
lines.push("> `Submission-RAISE-M2-HW1/prototype/test-results/report.json` — ห้ามแก้ด้วยมือ");
lines.push("> สร้างใหม่ได้ด้วย `npm test` ในโฟลเดอร์ `Submission-RAISE-M2-HW1/prototype/`");
lines.push("");
...
if (skippedTests.length) {
  ...
  lines.push("");
  lines.push("วิธีทำให้รันครบ — ในโฟลเดอร์ `Submission-RAISE-M2-HW1/prototype/`:");
  lines.push("");
  lines.push("```bash");
